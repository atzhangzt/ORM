...
namespace ServiceStack
{
    ...
    public static class PclExportExt
    {
        ...
        public static string ToFormUrlEncoded(this NameValueCollection queryParams)
        {
            ...
            foreach (string queryParam in (NameObjectCollectionBase)queryParams)
            {
                string[] values = queryParams.GetValues(queryParam);
                if (values != null)
                {
                    foreach (string text in values)
                    {
                        ...
                        stringBuilder.AppendFormat("{0}={1}", (object)queryParam.UrlEncode(false), (object)text.UrlEncode(false));
                    }
                }
            }
