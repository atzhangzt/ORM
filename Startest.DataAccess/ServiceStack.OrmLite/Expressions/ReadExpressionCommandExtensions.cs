...
namespace ServiceStack.OrmLite
{
    internal static class ReadExpressionCommandExtensions
    {
        ...
        internal static DataTable SelectTable<T>(this IDbCommand dbCmd, Expression<Func<T, bool>> predicate)
        {
            var expr = dbCmd.GetDialectProvider().SqlExpression<T>();
            string sql = expr.Where(predicate).SelectInto<T>();
            dbCmd.SetParameters(expr.Params);
            return dbCmd.Select(sql);
        }
        internal static DataTable Select(this IDbCommand dbCmd, string sql)
        {
