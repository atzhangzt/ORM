...
namespace ServiceStack.OrmLite
{
    public abstract partial class SqlExpression<T> : ISqlExpression, IHasUntypedSqlExpression
    {
        ...
        {
            string sql = DialectProvider.ToSelectStatement(modelDef, SelectExpression, BodyExpression, OrderByExpression, Offset, Rows);
            string strTemp = "SELECT " + DialectProvider.GetQuotedTableName(modelDef) +"."+ modelDef.PrimaryKey.Name + " FROM";
            int index = sql.IndexOf("FROM");
            sql = sql.Remove(0, index+4);
            sql = strTemp + sql;           
            
            if(sql.ToUpper().Contains("JOIN"))
            {
                return string.Format("DELETE FROM {0} WHERE {1} IN ({2})", DialectProvider.GetQuotedTableName(modelDef), modelDef.PrimaryKey.Name, sql);
            }
            else
            {
                return string.Format("DELETE FROM {0} {1}", DialectProvider.GetQuotedTableName(modelDef), whereExpression);
            }
        }
